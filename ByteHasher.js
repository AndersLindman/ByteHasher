const HASH_SIZE = 32;   // 256-bit output
const STATE_SIZE = 128; // 1024-bit state
...
// ------------------------------
// CONFUSION LAYER (non-linear mixing)
// ------------------------------
function confuse() {
...
// ------------------------------
// Sponge permutation = diffusion + confusion
// ------------------------------
function permute() {
    const ROUND_CONSTANTS = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80];
    for (let r = 0; r < 8; r++) {
...
function absorb(message) {
    ...
    for (let offset = 0; offset < message.length; offset += RATE) {
        const blockLen = Math.min(RATE, message.length - offset);

        for (let i = 0; i < blockLen; i++) {
            state[i] ^= rnd(message[offset + i]);
            pos++;
        }
...
function squeeze(outLen) {
    ...
    while (pos < outLen) {
        const take = Math.min(RATE, outLen - pos);
        out.set(state.slice(0, take), pos);
        pos += take;
        if (pos < outLen) {
            permute();
        }
...
// ------------------------------
// PADDING (Standard 10*1 Sponge Padding)
// ------------------------------
function pad(data) {
    const padLen = RATE - (data.length % RATE);
    const out = new Uint8Array(data.length + padLen);
    out.set(data);
    out[data.length] = 0x01;
...
function byteHistogramTest(samples = 100000, msgLen = 16) {
    ...
    const totalBytes = samples * HASH_SIZE;
...
function longMessageDiffusionTest(trials = 1000, numBlocks = 10) {
    ...
    const msgLen = numBlocks * RATE; // 10 blocks * 64 bytes = 640 bytes
    const outputBits = HASH_SIZE * 8; // 256 bits
    ...
    for (let t = 0; t < trials; t++) {
        ...
        const byteIndex = Math.floor(rng() * RATE);
...
function lowEntropyStressTest(trials = 100) {
    ...
    for (const p of patterns) {
        ...
        for (let t = 0; t < trials; t++) {
            ...
            totalFlipRatio += bitCount(diff) / (HASH_SIZE * 8);
        }
